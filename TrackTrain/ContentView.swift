...
struct ContentView: View {
    ...
    let locationsStation: [Town] = [Town(id: "", name: "RHB Berguen", coordinates: [46.631158, 9.746958])]
    @ObservedObject var websocket = TrainWebsocket()
    
    @State private var position: MapCameraPosition = .automatic
    @State private var visibleRegion: MKCoordinateRegion?
    @State private var selectedItem: String?
    @State var selectedRoute: LocationTrain?
    @State var selectedStops: TrainStopContent?
    @State var selectedTrajectory: [CLLocationCoordinate2D]?
    ...
    var body: some View {
        Map(position: $position, selection: $selectedItem) {
            ...
            if (selectedRoute != nil) {
                MapPolyline(coordinates: selectedTrajectory ?? routeLine, contourStyle: .geodesic)
                    .stroke(.blue, style: StrokeStyle(lineWidth: 5, lineCap: .round, lineJoin: .round))
            }
            
            UserAnnotation()
        }.safeAreaInset(edge: .bottom, content: {
            BottomView(selectedTrajectory: $selectedTrajectory, selectedRoute: $selectedRoute, selectedStops: $selectedStops, visibleRegion: $visibleRegion, position: $position, selectedItem: $selectedItem, websocket: websocket)
        })
        ...
        .onChange(of: selectedItem ?? "nothing", { _, trainSelectedOnMap in
            print("Selected train \(trainSelectedOnMap)")
            websocket.getStopsTrains(trainSelectedOnMap)
            websocket.getFullTarjactoryTrains(selectedItem ?? "nothing")
            let trainIndex = websocket.locations.firstIndex(where: {$0.id == selectedItem})
            selectedRoute = websocket.locations[trainIndex!]
            print("trajectory.count: \(websocket.locations[trainIndex!].trajectory.count) and trajectoryExpanded.count: \(websocket.locations[trainIndex!].trajectoryExpanded.count)")
            print("destination: \(String(describing: selectedRoute?.stops?.destination))")
            position = .automatic
        })
        .onAppear {
